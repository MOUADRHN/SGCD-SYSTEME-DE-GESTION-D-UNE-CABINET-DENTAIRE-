...
<div class="main-content">
    ...
    <div class="row g-3">
        <%-- ─── 1. COLONNE GAUCHE : Résumé clinique ───────────────────────── --%>
        <div class="col-md-7">
            <div class="card h-100">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="p-2 rounded border" style="background:#f8f9fa; white-space:pre-wrap;"><c:out value="${cons.diagnostic}"/></div>
                    </div>
                    <c:if test="${not empty cons.observations}">
                        <div class="mb-3">
                            <div class="text-muted small fw-semibold text-uppercase mb-1">Observations</div>
                            <div class="p-2 rounded border" style="background:#f8f9fa; white-space:pre-wrap;"><c:out value="${cons.observations}"/></div>
                        </div>
                    </c:if>
                                ...
                                <table class="table table-sm table-bordered mb-0">
                                    ...
                                    <tr class="table-active fw-bold">
                                        ...
                                        <td class="text-end text-success">
                                            <fmt:formatNumber value="${cons.calculerMontantTotal()}" maxFractionDigits="0"/> MAD
                                        </td>
                                    </tr>
                                    </tfoot>
                                </table>
        ...
        <%-- ─── 2. COLONNE DROITE : Prescription + Facture ────────────────── --%>
                    ...
                    <c:choose>
                        <c:when test="${not empty prescription}">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div class="text-muted small">Date : ${prescription.date}</div>
                                    ...
                                    <form method="post" action="${pageContext.request.contextPath}/consultation">
                                        ...
                                        <div id="meds-list">
                                            <div class="med-row border rounded p-2 mb-2 bg-light">
                                                <div class="row g-1">
                                                    <div class="col-5">
                                                        <input type="text" name="med_nom" class="form-control form-control-sm" placeholder="Médicament" required>
                                                    </div>
                                                    <div class="col-4">
                                                        <input type="text" name="med_dosage" class="form-control form-control-sm" placeholder="Dosage">
                                                    </div>
                                                    <div class="col-3">
                                                        <input type="number" name="med_duree" class="form-control form-control-sm" placeholder="Jrs" min="1" value="7">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="d-flex gap-2 mb-2">
                                            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="addMed()">
                    ...
                    <div class="card-body">
                        ...
                        <c:if test="${facture.paiement != null}">
                            <div class="text-muted small mt-1">
                                Payé le ${facture.paiement.datePaiement} — ${facture.paiement.modePaiement.libelle}
                            </div>
                        </c:if>
                        <c:if test="${sessionScope.utilisateur.role != 'DENTISTE'}">
                        <a href="${pageContext.request.contextPath}/facture?action=detail&id=${facture.idFacture}" class="btn btn-sm btn-mint w-100 mt-2">
